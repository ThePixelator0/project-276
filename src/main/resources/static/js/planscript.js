...
document.addEventListener('DOMContentLoaded', () => {
  const appId = "51cdd3fc";
  const appKey = "14ef4e66e05b6443e3a29a1c89cd5ef8";

  const apiUrl = 'https://api.edamam.com/auto-complete';

  const foodNameInput = document.getElementById('foodName');

  let timeoutId; // To debounce requests

  foodNameInput.addEventListener('input', () => {
    const query = foodNameInput.value.trim();

    if (query.length >= 2) {
      // Clear previous timeout (if any) to debounce requests
      clearTimeout(timeoutId);

      // Debounce the fetch to avoid making requests on every key press
      timeoutId = setTimeout(() => {
        fetch(`${apiUrl}?app_id=${appId}&app_key=${appKey}&q=${query}`)
      ...
      }, 300); // Wait for 300ms after the user stops typing to make the request
    }
  });

  function displaySuggestions(suggestions) {
    const suggestionsList = suggestions.map(food => `<div>${food}</div>`).join('');
    foodSuggestionsContainer.innerHTML = suggestionsList;
  }
});
